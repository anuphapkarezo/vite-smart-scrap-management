...
export default function Scrap_Monthly_Monitoring_by_buyer({ onSearch }) {
  ...
  const fetchBarChartData_Weight = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`http://10.17.100.115:3001/api/smart_scrap/filter-data-monthly-monitoring-buyer-chart-weight?from_year=${selectedFromYear}&to_year=${selectedToYear}`);
      const data = await response.data;
  ...
  const fetchPieChartData = async () => {
    try {
        ...
        const response = await axios.get(`http://10.17.100.115:3001/api/smart_scrap/filter-data-monthly-monitoring-buyer-pie?from_year=${selectedFromYear}&to_year=${selectedToYear}`);
        ...
        const labels = data.map(item => item.waste_company_short_name); // Extract labels from waste_company_short_name
  ...
  const fetchPieChartData_Weight = async () => {
    try {
        ...
        const response = await axios.get(`http://10.17.100.115:3001/api/smart_scrap/filter-data-monthly-monitoring-buyer-pie-weight?from_year=${selectedFromYear}&to_year=${selectedToYear}`);
  ...
  const fetchBasicBarChartData = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`http://10.17.100.115:3001/api/smart_scrap/filter-data-monthly-monitoring-buyer-total?from_year=${selectedFromYear}&to_year=${selectedToYear}`);
      const data = response.data;
  ...
  const fetchBasicBarChartData_Weight = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`http://10.17.100.115:3001/api/smart_scrap/filter-data-monthly-monitoring-buyer-total-weight?from_year=${selectedFromYear}&to_year=${selectedToYear}`);
      const data = response.data;
    ...
    <>
        ...
        <Box marginLeft={isNavbarOpen ? "220px" : 4} marginTop={10}>
          <div className="w-screen ml-16 mt-20">
              ...
              <Box sx={{width: 1050 , height: 550 , marginTop: '25px'}}>
                {isLoading ? (
              ...
              {/* <div style={{ display: 'flex', flexDirection: 'row'}}>
                ...
                <Box sx={{ width: 550, height: 400, marginTop: '30px', backgroundColor: '#EEF5FF' , marginLeft: '20px' }}>
